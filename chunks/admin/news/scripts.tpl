...
var currPage = parseInt($("#currPage").val());
    $(document).ready(function() {
        ...
        $("#addGLI").submit(function(event) {
            ...
            var isActive = 0;
            if ($('#isActive').is(':checked')) { isActive = 1; }
            values.append('isActive', isActive); 

            // var isFeature = 0;
            // if ($('#isFeature').is(':checked')) { isFeature = 1; }
            // values.append('isFeature', isFeature); 
        ...
        $("#editGLI").submit(function(event) {
    	   event.preventDefault();
            ...
            var editIsActive = 0;
            if ($('#edit_isActive').is(':checked')) { editIsActive = 1; }
            values.append('edit_isActive', editIsActive); 

            // var editIsFeature = 0;
            // if ($('#edit_isFeature').is(':checked')) { editIsFeature = 1; }
            // values.append('edit_isFeature', editIsFeature);

            var editInHome = 0;
            if ($('#edit_inHome').is(':checked')) { editInHome = 1; }
        ...
        $.ajax({
            ...
            success: function(item) {
                item = $.parseJSON(item);
                    ...
                    // if (item.IsFeature == 1) {
                    //     $('#edit_isFeature').prop('checked', true); }
