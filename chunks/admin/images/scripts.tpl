...
var currPage = parseInt($("#currPage").val());
    $(document).ready(function() {
        ...
        $("#addGL").submit(function(event) {
            ...
            $.ajax({
                url: "../handlers/ImagesHandler.php",
                type: "post",
                data: values,
                cache: false,
                contentType: false,
    ...
    function fnUploadPhotoModal() {
        ...
        .on('submitted', function (event, id, name) {
            ...
            extraInputsHTML += '<input type="text" name="title_ar">';
            extraInputsHTML += '<label>Title AR</label>';
            extraInputsHTML += '</div>';
            extraInputsHTML += '<div class="input-field col s6">';
            extraInputsHTML += '<input type="text" name="title_en">';
            extraInputsHTML += '<label>Title EN</label>';
            extraInputsHTML += '</div>';
            extraInputsHTML += '</div>';
        ...
        .on('upload', function (event, id, name) {
            ...
            var title_ar = $(fileItemContainer).find('input[name="title_ar"]').val();
            var title_en = $(fileItemContainer).find('input[name="title_en"]').val();

            $(this).fineUploader('setParams', {
                title_ar: title_ar,
                title_en: title_en,
                operation: 'uploadAlbumPhotos',
            }, id);
        })
        .on('allComplete', function (event, succeeded, failed) {
            Materialize.toast('Pictures have been uploaded successfully', 2000);
            currPage = 1;
